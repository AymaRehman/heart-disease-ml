...
# normalisation is needed because features have very different scales
# for example age is 29-77 but chol is 126-564
# minmax scaling brings everything between 0 and 1
...
print("Normalisation ")
print("MinMax scaling applied to continuous features")
print(f"Continuous features scaled: {continuous_features}")
...
plt.legend()
plt.tight_layout()
plt.savefig("data/exp1_dendrogram.png")
plt.show()
...
# raising the cutoff to 5 means we cut higher up the tree
# so groups that were separate in exp1 now merge together
# this gives us fewer but bigger clusters

plt.figure(figsize=(12, 6))
dendrogram(linkage_matrix)
plt.title("Hierarchical Clustering Dendrogram - Experiment 2 (cutoff=5)")
plt.xlabel("Patient Index")
plt.ylabel("Distance")
plt.axhline(y=5, color="g", linestyle="--", label="cutoff = 5")
# green line so we can visually tell experiments apart in screenshots
plt.legend()
plt.tight_layout()
plt.savefig("data/exp2_dendrogram.png")
plt.show()

# same linkage matrix as exp1 , only the cutoff changes across all experiments
clusters_exp2 = fcluster(linkage_matrix, t=5, criterion="distance")

print("=== Experiment 2 (cutoff = 5) ===")
...
# cutting at 8 means we go very high up the tree


plt.figure(figsize=(12, 6))
dendrogram(linkage_matrix)
plt.title("Hierarchical Clustering Dendrogram - Experiment 3 (cutoff=8)")
plt.xlabel("Patient Index")
plt.ylabel("Distance")
plt.axhline(y=8, color="b", linestyle="--", label="cutoff = 8")
# blue line to distinguish from exp1 (red) and exp2 (green)
plt.legend()
plt.tight_layout()
plt.savefig("data/exp3_dendrogram.png")
plt.show()

# same linkage matrix, only cutoff value changes
clusters_exp3 = fcluster(linkage_matrix, t=8, criterion="distance")

print("=== Experiment 3 (cutoff = 8) ===")
...
print(f"Experiment 2 (cutoff = 5): {round(score2, 4)}")
print(f"Experiment 3 (cutoff = 8): {round(score3, 4)}")

# note: the one with highest silhouette score is best experiment 
best_cutoff = {score1: 3, score2: 5, score3: 8}
best_score = max(score1, score2, score3)
print(f"\nBest cutoff: {best_cutoff[best_score]} with silhouette score: {round(best_score, 4)}")
...
print(f"Experiment 2 (cutoff = 5): {len(set(clusters_exp2))} clusters formed")
print(f"Experiment 3 (cutoff = 8): {len(set(clusters_exp3))} clusters formed")
print()
...
print(f"Best performing experiment had silhouette score of: {round(best_score, 4)}")